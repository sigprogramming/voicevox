...
export const singingStore = createPartialStore<SingingStoreTypes>({
  ...
  /**
   * 指定された歌声データからオーディオシーケンスを非同期で生成して登録する。
   * 既存のシーケンスが存在する場合は、そのシーケンスを削除してから生成したオーディオシーケンスを登録する。
   * オーディオシーケンス生成中にフレーズが無くなったり歌声が変わった場合は、シーケンスの削除・登録は行わない。
   */
  GENERATE_AND_REGISTER_PHRASE_AUDIO_SEQUENCE: {
    async action(
      { mutations, state },
      { startTime, singingVoiceKey, singingVoice, phraseKey, trackId },
    ) {
      // まず最初に、非同期処理であるオーディオシーケンスの生成処理を行う
      ...
      // オーディオシーケンスを生成している間にフレーズが無くなったり歌声が変わった場合は、以降の処理をキャンセルする
      const phrase = state.phrases.get(phraseKey);
      if (phrase == undefined || phrase.singingVoiceKey !== singingVoiceKey) {
        return;
      }

      // 既存のシーケンスが存在する場合、シーケンスを削除する
      if (phrase.sequenceId != undefined) {
        deleteSequence(phrase.sequenceId);
  ...
  /**
   * SongTrackRendererの生成とイベントリスナーの登録を行う。
   */
  SETUP_SONG_TRACK_RENDERER: {
    async action({ state, actions, mutations }) {
      ...
      const onCacheLoaded = (event: CacheLoadedEvent) => {
        ...
        for (const [phraseKey, eventPhrase] of event.phrases) {
          ...
          const newPhrase: Phrase = {
            ...
            sequenceId:
              existingPhrase == undefined // 以前のフレーズが存在しない -> 新規フレーズなので undefined
                ? undefined
                : existingPhrase.singingVoiceKey !== eventPhrase.singingVoiceKey // 歌声が変わった -> シーケンス再生成が必要なので undefined
                  ? undefined
                  : existingPhrase.sequenceId, // 歌声が同じ -> 既存のシーケンスIDを引き継ぐ
        ...
        for (const [phraseKey, phrase] of state.phrases) {
          const newPhrase = newPhrases.get(phraseKey);
          if (
            phrase.sequenceId != undefined &&
            (newPhrase == undefined || newPhrase.sequenceId == undefined)
          ) {
            deleteSequence(phrase.sequenceId);
          }
        }
        ...
        // 更新後の各フレーズの状態に基づいてシーケンスの削除・生成・登録を行う
        for (const [phraseKey, phrase] of state.phrases.entries()) {
          // 状態が 'RENDERED' で、かつシーケンスが存在しないフレーズ（キャッシュがロードされたフレーズ）は、
          // オーディオシーケンスを生成してフレーズの音声が再生されるようにする
          if (phrase.state === "RENDERED" && phrase.sequenceId == undefined) {
            ...
            // オーディオシーケンスを生成して登録し、フレーズの音声が再生されるようにする
            void actions.GENERATE_AND_REGISTER_PHRASE_AUDIO_SEQUENCE({
          ...
          // シンガー未設定 または レンダリング待ち のフレーズは、
          // ノートシーケンスを生成してプレビュー音で再生されるようにする
          if (
            phrase.state === "SINGER_IS_NOT_SET" ||
            phrase.state === "WAITING_TO_BE_RENDERED"
          ) {
            // 既存のシーケンスが存在する場合は削除する
            if (phrase.sequenceId != undefined) {
              deleteSequence(phrase.sequenceId);
              mutations.SET_SEQUENCE_ID_TO_PHRASE({
                phraseKey,
            ...
            // ノートシーケンスを生成して登録し、プレビュー音で再生されるようにする
            const sequenceId = SequenceId(uuid4());
      ...
      const onPhraseRenderingComplete = (
        ...
        // オーディオシーケンスを生成して登録する
        void actions.GENERATE_AND_REGISTER_PHRASE_AUDIO_SEQUENCE({
          startTime: event.phrase.startTime,
          singingVoiceKey: singingVoiceKey,
          singingVoice: singingVoice,
          phraseKey: event.phraseKey,
          trackId: event.phrase.trackId,
        });
      ...
      // --- SongTrackRenderer のインスタンス化とリスナー登録 ---

      // SongTrackRenderer を初期化
  ...
  RENDER: {
    async action({ state, getters, mutations, actions }) {
      ...
      if (songTrackRenderer == undefined) {
        void actions.SETUP_SONG_TRACK_RENDERER();
        if (songTrackRenderer == undefined) {
