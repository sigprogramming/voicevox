...
const render = () => {
  ...
  for (const note of notes) {
    // 位置とサイズを計算
    const baseStartX = tickToBaseX(note.position, tpqn.value);
    const baseEndX = tickToBaseX(note.position + note.duration, tpqn.value);
    ...
    // ノートの歌詞をテキストで描画
    const lyric = note.lyric ?? getDoremiFromNoteNumber(note.noteNumber);
