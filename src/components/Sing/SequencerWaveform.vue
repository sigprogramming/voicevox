...
watch(waveformDataSources, async () => {
  try {
...
const render = () => {
  ...
  for (const waveformData of waveformDataMap.value.values()) {
    const startTicks = waveformData.startTicks;
    const endTicks = waveformData.endTicks;
    ...
    drawWaveform(
      ...
      startScreenX,
      canvasHeight,
      waveformColors.value.color,
      waveformColors.value.alpha,
    );
  }
